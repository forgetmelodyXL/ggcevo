...
| 指令 | 别名 | 权限 | 说明 |
| --- | --- | --- | --- |
| `咕咕之战` | 菜单 | — | 生成咕咕之战内容分类菜单（「每日玩法」「物品背包」「抽奖兑换」「活动」「句柄绑定」「查询信息」六类，仅收录普通用户可用指令）。根据配置项 `menuStyle` 选择渲染方式：`text`（默认）输出文字分类菜单，`image` 使用 puppeteer 渲染分类图片菜单（需启用 puppeteer 服务，未启用时自动降级为文字菜单） |
