...
| `banNotifyGroups` | `string[]` | `[]` | 封禁提醒广播的群组ID列表。同步检测到新增封禁记录时在此提醒：句柄绑定了QQ则 @该用户，未绑定则只提醒句柄（含封禁等级、审核员、审核时间，处罚原因详情见封禁文档） |
...
| `封禁记录 [user]` | — | — | 查询封禁记录。不带参数查询自己，携带 @用户 查询对方句柄的封禁记录。每页显示 1 条，支持翻页（回复"下一页/上一页/页码"翻页，回复其他任意内容退出）。审核时间仅显示年月日 |
