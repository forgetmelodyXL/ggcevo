...
| `菜单` | ggcevo指令 | 渲染 GGCEVO 句柄管理指令菜单图片（绑定/句柄/切换/查询/解绑/地图检测/封禁记录）。需启用 `koishi-plugin-puppeteer` 渲染 |

### 签到与虚拟物品（ggcevo）

| 指令 | 别名 | 权限 | 说明 |
| --- | --- | --- | --- |
| `咕咕之战` | — | — | 渲染咕咕之战玩法指令菜单图片（签到/抽奖/兑换/背包/个人信息/兑换列表/活动列表/领取活动/补签/使用/挖矿/探索）。需启用 `koishi-plugin-puppeteer` 渲染 |
...
- **声明 puppeteer 服务依赖**：插件现可调用 `koishi-plugin-puppeteer` 服务（可选依赖，未安装时不影响其他功能），需安装启用 `koishi-plugin-puppeteer`（^3.9.0）
