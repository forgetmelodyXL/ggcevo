...
| 指令 | 别名 | 权限 | 说明 |
| --- | --- | --- | --- |
| `咕咕之战` | 菜单 | — | 生成咕咕之战内容分类菜单（「每日玩法」「物品背包」「抽奖兑换」「活动」「句柄绑定」「查询信息」六类，仅收录普通用户可用指令）。QQ 官方机器人下使用 Markdown+按钮渲染，点击分类按钮进入子菜单、点击指令按钮直接执行（兑换/使用/绑定/迁移等需参数的指令支持输入框按钮）；其余平台自动回退为纯文本菜单 |
